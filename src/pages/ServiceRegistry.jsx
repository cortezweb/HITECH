    ...
    return (
      <div className="space-y-6 text-left">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-outline-variant/10 pb-6">
          ...
          {/* Stepper Header */}
          <div className="flex items-center gap-1.5 sm:gap-2 bg-surface-container-low p-1.5 sm:p-2 rounded-2xl border border-outline-variant/20 overflow-x-auto no-scrollbar max-w-full">
            <div className={`flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3 py-1.5 rounded-xl text-[11px] sm:text-[12px] font-bold whitespace-nowrap ${step >= 1 ? 'bg-white shadow-sm text-primary' : 'text-on-surface-variant/40'}`}>
              <span className="material-symbols-outlined text-[16px]">{step > 1 ? 'check_circle' : 'person'}</span>
              <span>Cliente</span>
            </div>
            <div className="w-3 sm:w-6 h-[2px] bg-outline-variant/30 flex-shrink-0"></div>
            <div className={`flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3 py-1.5 rounded-xl text-[11px] sm:text-[12px] font-bold whitespace-nowrap ${step >= 2 ? 'bg-white shadow-sm text-primary' : 'text-on-surface-variant/45'}`}>
              <span className="material-symbols-outlined text-[16px]">{step > 2 ? 'check_circle' : 'devices'}</span>
              <span>Equipo</span>
            </div>
            <div className="w-3 sm:w-6 h-[2px] bg-outline-variant/30 flex-shrink-0"></div>
            <div className={`flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3 py-1.5 rounded-xl text-[11px] sm:text-[12px] font-bold whitespace-nowrap ${step === 3 ? 'bg-white shadow-sm text-primary' : 'text-on-surface-variant/45'}`}>
        ...
        <div className="bg-white p-4 sm:p-8 rounded-2xl sm:rounded-3xl shadow-sm border border-outline-variant/10 max-w-4xl mx-auto">
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-3 sm:gap-4 mb-6">
        <div>
          <h2 className="text-[20px] sm:text-[24px] font-black text-on-surface leading-tight">Órdenes de Servicio</h2>
          <p className="text-[12px] sm:text-[13px] text-on-surface-variant mt-0.5 sm:mt-1">Administra los tickets de mantenimiento técnico</p>
        </div>
        ...
        <div className="flex flex-wrap sm:flex-nowrap gap-2 sm:gap-3 w-full sm:w-auto">
          <button
            ...
            className="flex-1 sm:flex-none justify-center flex items-center gap-1.5 px-3.5 sm:px-5 py-2.5 bg-surface-container hover:bg-surface-container-high border border-outline-variant/30 text-on-surface rounded-xl font-semibold text-[12px] sm:text-[13px] hover:brightness-105 active:scale-95 transition-all cursor-pointer"
          >
          ...
          <button
            ...
            className="flex-1 sm:flex-none justify-center flex items-center gap-1.5 px-3.5 sm:px-5 py-2.5 bg-primary text-on-primary rounded-xl font-semibold text-[12px] sm:text-[13px] hover:brightness-105 active:scale-95 transition-all cursor-pointer"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-8 space-y-4">
          <div className="bg-white p-4 sm:p-card-padding rounded-[20px] shadow-sm border border-outline-variant/10 space-y-4">
            <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center justify-between border-b border-outline-variant/10 pb-4 mb-4">
              <h3 className="text-[15px] sm:text-[16px] font-bold">Mesa de Servicio</h3>
              ...
              {/* Filtros de Estado */}
              <div className="flex gap-1 overflow-x-auto no-scrollbar pb-1 sm:pb-0 whitespace-nowrap">
                {['All', 'Pending', 'In Progress', 'Completed'].map((status) => (
                  <button
                    key={status}
                    type="button"
                    onClick={() => setStatusFilter(status)}
                    className={`px-3 py-1.5 rounded-xl text-[10px] font-bold border transition-all cursor-pointer whitespace-nowrap flex-shrink-0 ${
                      statusFilter === status
            ...
            <div className="divide-y divide-outline-variant/10">
              {tickets
                ...
                <div
                  ...
                  className={`py-3 sm:py-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 cursor-pointer group rounded-xl p-3 sm:px-4 hover:bg-surface-container-low transition-all ${
                    selectedTicket?.id === ticket.id ? 'bg-primary/5 border border-primary/10' : ''
                  }`}
                >
                  <div className="flex items-start gap-3 sm:gap-4 w-full sm:w-auto">
                    <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-xl bg-surface-container overflow-hidden p-1.5 flex-shrink-0 flex items-center justify-center">
                      <img className="w-full h-full object-cover rounded-lg" alt={ticket.address} src={ticket.image} />
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-bold text-[13px] sm:text-[14px] text-on-surface truncate">{ticket.systemType}</span>
                        <span className="font-mono text-[10px] sm:text-[11px] text-primary bg-primary-fixed/20 px-2 py-0.5 rounded-full font-bold">{ticket.id}</span>
                      </div>
                      <div className="flex items-center gap-2 flex-wrap mt-0.5">
                        <p className="text-[11px] sm:text-[12px] text-on-surface-variant truncate">Cliente: {ticket.address} • {ticket.city}</p>
                        {ticket.assignedTech && (
                  ...
                  <div className="flex items-center justify-between sm:justify-end w-full sm:w-auto gap-2 sm:gap-3 border-t sm:border-t-0 border-slate-100 pt-2 sm:pt-0">
                    <button
                      type="button"
                      onClick={(e) => {
                        e.stopPropagation();
                    ...
                    <div className="text-right">
                      <p className="text-[13px] sm:text-[14px] font-black text-on-surface">Bs. {ticket.price.toFixed(2)}</p>
                      <p className="text-[10px] sm:text-[11px] text-on-surface-variant">{ticket.date}</p>
                    </div>
      ...
      {previewReceiptTicket && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex flex-col items-center justify-center p-4 print:hidden overflow-y-auto animate-in fade-in duration-200">
          <div className="bg-slate-100 rounded-3xl shadow-2xl max-w-4xl w-full max-h-[95vh] flex flex-col overflow-hidden border border-slate-300">
            {/* Modal Header */}
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 px-4 sm:px-6 py-3 sm:py-4 bg-[#0e2a47] text-white">
              <div className="flex items-center gap-2">
                <span className="material-symbols-outlined text-[24px] text-amber-400 flex-shrink-0">description</span>
                <div>
                  <h3 className="text-[14px] sm:text-[16px] font-black uppercase tracking-wide leading-tight">
              ...
              <div className="flex items-center justify-end w-full sm:w-auto gap-2 sm:gap-3">
                <button
                  ...
                  className="flex-1 sm:flex-none justify-center px-3.5 sm:px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-[11px] sm:text-[12px] font-bold flex items-center gap-1.5 shadow transition-all cursor-pointer"
                >
                  <span className="material-symbols-outlined text-[17px]">print</span>
            ...
            <div className="flex-1 overflow-auto p-2 sm:p-6 flex justify-center bg-slate-200/70">
              <div className="bg-white shadow-2xl rounded-sm border border-slate-300 transform scale-[0.65] sm:scale-95 origin-top max-w-full">
